...
// Cloudinary — direct browser uploads (bypasses Vercel body limit)
...
async function handleApi(req, res) {
  try {
    ...
    if (req.method === "POST" && pathname === "/api/telegram/order") return telegramOrder(req, res);
    if (req.method === "GET" && pathname === "/api/config") return apiConfig(req, res);
...
async function telegramOrder(req, res) {
  ...
  if (!telegramBotToken || !telegramChatId) {
    return json(res, 503, { error: "Telegram bot not configured. Set TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID in environment variables." });
  }
  ...
  if (!tgRes.ok) return json(res, 502, { error: tgData.description || "Telegram API error" });
  json(res, 200, { ok: true });
}

function apiConfig(req, res) {
  // These values are safe to expose — unsigned Cloudinary presets are designed to be public
  json(res, 200, {
    cloudinaryCloudName,
