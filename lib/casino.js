...
const SLOT_V2_SYMBOLS = [
  ...
  { id: "scatter", icon: "🧭", name: "Scatter Compass", type: "scatter", weight: 4, payout: [0, 0, 2, 10, 40, 100] }
];
  ...
  for (const key of Object.keys(def)) {
    if (key === "wheel") {
      ...
      out.slotsV2 = {
        ...
        weights: { ...def.slotsV2.weights, ...(stored.slotsV2?.weights || {}) },
...
async function playSlotsV2(req, res) {
  ...
  if (scatters >= 3) {
    awardedFreeSpins = cfg.slotsV2.freeSpinsAward;
    player.slotV2FreeSpins = (player.slotV2FreeSpins || 0) + awardedFreeSpins;
    player.slotV2FreeSpinMultiplier = Math.max(player.slotV2FreeSpinMultiplier || 1, 1);
  }
  if (!player.slotV2FreeSpins) player.slotV2FreeSpinMultiplier = 1;
  applySlotRound(player, "slotsV2", bet, payout, freeSpin, `${scatters} scatter · ${evaluated.lineWins.length} lines`);
...
function buildSlotV2Strip(cfg) {
  const strip = [];
  for (const id of SLOT_V2_SYMBOL_IDS) {
    const w = Math.max(0, Math.floor(cfg.slotsV2.weights[id] || 0));
