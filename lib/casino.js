...
const SLOT_V2_SYMBOLS = [
  { id: "grape", icon: "🍇", asset: slotV2Asset("grape"), name: "Grapes", type: "normal", weight: 28, payout: [0, 0, 1, 3, 12, 45] },
  { id: "watermelon", icon: "🍉", asset: slotV2Asset("watermelon"), name: "Watermelon", type: "normal", weight: 25, payout: [0, 0, 1, 4, 14, 55] },
  { id: "apple", icon: "🍎", asset: slotV2Asset("apple"), name: "Apple", type: "normal", weight: 23, payout: [0, 0, 1, 5, 18, 70] },
  { id: "lemon", icon: "🍋", asset: slotV2Asset("lemon"), name: "Lemon", type: "normal", weight: 21, payout: [0, 0, 1, 6, 22, 85] },
  { id: "cherry", icon: "🍒", asset: slotV2Asset("cherry"), name: "Cherry", type: "normal", weight: 19, payout: [0, 0, 1, 7, 26, 100] },
  { id: "orange", icon: "🍊", asset: slotV2Asset("orange"), name: "Orange", type: "normal", weight: 18, payout: [0, 0, 1, 8, 30, 120] },
  { id: "star", icon: "★", asset: slotV2Asset("star"), name: "Golden Star", type: "normal", weight: 11, payout: [0, 0, 2, 10, 42, 170] },
  { id: "seven", icon: "7", asset: slotV2Asset("seven"), name: "Lucky Seven", type: "normal", weight: 5, payout: [0, 0, 3, 16, 80, 320] },
  { id: "bell", icon: "🔔", asset: slotV2Asset("bell"), name: "Bell", type: "normal", weight: 8, payout: [0, 0, 2, 13, 58, 240] },
  { id: "plum", icon: "🟣", asset: slotV2Asset("plum"), name: "Plum", type: "normal", weight: 16, payout: [0, 0, 1, 9, 34, 135] },
  { id: "wild", icon: "WILD", asset: slotV2Asset("wild"), name: "Wild Crown", type: "wild", weight: 2, payout: [0, 0, 0, 0, 0, 0] },
  { id: "scatter", icon: "$", asset: slotV2Asset("scatter"), name: "Scatter Coin", type: "scatter", weight: 3, payout: [0, 0, 0, 4, 18, 60] },
  { id: "bonus", icon: "BONUS", asset: slotV2Asset("bonus"), name: "Bonus Diamond", type: "bonus", weight: 2, payout: [0, 0, 0, 0, 0, 0] },
  { id: "goodluck", icon: "LUCK", asset: slotV2Asset("goodluck"), name: "Good Luck", type: "luck", weight: 2, payout: [0, 0, 0, 1, 4, 14] }
];
...
const SHOP_ITEMS = [
  { id: "scatterCharm", name: "Diamond Charm", cost: 900, spins: 14, effect: { scatterBoost: 2 }, icon: "💎" },
  { id: "wildCharm", name: "Wild Spark", cost: 1400, spins: 10, effect: { wildBoost: 2 }, icon: "⭐" },
  { id: "lossShield", name: "Loss Shield", cost: 900, spins: 12, effect: { lossRefundPercent: 25 }, icon: "🛡️" },
  { id: "multiplierGlow", name: "Multiplier Glow", cost: 1800, spins: 8, effect: { winMultiplierBoost: 12 }, icon: "✨" },
  { id: "bonusMagnet", name: "Bonus Magnet", cost: 3500, spins: 8, effect: { scatterBoost: 3, wildBoost: 1 }, icon: "🧲" }
];
...
function defaultConfig() {
  return {
    ...
    slotsV2: {
      ...
      rows: 3,
      freeSpinsAward: 5,
      freeSpinMultiplierStep: 0.5,
      freeSpinMaxMultiplier: 3,
      bonusBuys: [
        { id: "starter", cost: 1500, freeSpins: 4, multiplier: 1.5 },
        { id: "plus", cost: 3500, freeSpins: 6, multiplier: 2 },
        { id: "mega", cost: 9000, freeSpins: 10, multiplier: 2.5 }
      ],
  ...
  for (const key of Object.keys(def)) {
    if (key === "wheel") {
      ...
      for (const id of SLOT_V2_SYMBOL_IDS) {
        weights[id] = id === "scatter"
          ? Math.max(num(storedWeights.scatter, def.slotsV2.weights.scatter), def.slotsV2.weights.scatter)
          : num(storedWeights[id], def.slotsV2.weights[id]);
        payouts[id] = Array.isArray(stored.slotsV2?.payouts?.[id]) ? stored.slotsV2.payouts[id] : def.slotsV2.payouts[id];
      }
      out.slotsV2 = {
        reels: 5,
        rows: 3,
...
async function playSlotsV2(req, res) {
  ...
  const bonusSymbols = evaluated.bonusSymbols.length;
  const luckSymbols = evaluated.luckSymbols.length;
  ...
  applySlotRound(player, "slotsV2", bet, payout, freeSpin, `${scatters} scatter · ${bonusSymbols} bonus · ${luckSymbols} luck · ${evaluated.lineWins.length} lines${shieldRefund ? ` · shield +${shieldRefund}` : ""}`);
  ...
  json(res, 200, {
    ...
    outcome: {
      ...
      bonusWin: evaluated.bonusWin,
      luckWin: evaluated.luckWin,
      scatters: evaluated.scatters,
      bonusSymbols: evaluated.bonusSymbols,
      luckSymbols: evaluated.luckSymbols,
      freeSpin,
...
function sanitizeConfig(input) {
  ...
  hardenSlotV2Economy(merged.slotsV2);
  capSlotV2Rtp(merged.slotsV2, 0.68);
...
function hardenSlotV2Economy(slots) {
  const maxWeights = { wild: 2, scatter: 3, bonus: 2, goodluck: 2, seven: 5, bell: 8, star: 11 };
  for (const [id, max] of Object.entries(maxWeights)) {
    slots.weights[id] = Math.min(Number(slots.weights[id]) || max, max);
  }
  for (const id of SLOT_V2_SYMBOL_IDS) {
    const maxPayout = SLOT_V2_SYMBOL_BY_ID[id].payout || [0, 0, 0, 0, 0, 0];
    slots.payouts[id] = [0, 0, 0, 0, 0, 0].map((_, i) => Math.min(Number(slots.payouts[id]?.[i]) || 0, maxPayout[i] || 0));
  }
  slots.freeSpinsAward = Math.min(slots.freeSpinsAward, 5);
  slots.freeSpinMultiplierStep = Math.min(slots.freeSpinMultiplierStep, 0.5);
  slots.freeSpinMaxMultiplier = Math.min(slots.freeSpinMaxMultiplier, 3);
  slots.bonusBuys = slots.bonusBuys.map((pack) => {
    const freeSpins = Math.min(pack.freeSpins, pack.id === "mega" ? 10 : pack.id === "plus" ? 6 : 4);
    const multiplier = Math.min(pack.multiplier, pack.id === "mega" ? 2.5 : pack.id === "plus" ? 2 : 1.5);
    const floor = Math.ceil(freeSpins * multiplier * 600);
    return { ...pack, freeSpins, multiplier, cost: Math.max(pack.cost, floor) };
  });
}
...
function createSlotV2Bonus(bet, cfg) {
  ...
  const prizes = [
    ...
    { type: "chips", freeSpins: Math.max(2, Math.floor(baseSpins / 2)), chips: Math.max(5, bet * (2 + rngInt(0, 3))), multiplierBoost: 0 },
    { type: "multiplier", freeSpins: Math.max(2, baseSpins - 1), chips: 0, multiplierBoost: rngInt(0, 2) }
  ];
...
function evaluateSlotsV2(grid, cfg, bet, freeSpinMultiplier) {
  ...
  for (let reel = 0; reel < grid.length; reel++) {
    for (let row = 0; row < grid[reel].length; row++) {
      ...
      if (grid[reel][row] === "bonus") bonusSymbols.push([reel, row]);
      if (grid[reel][row] === "goodluck") luckSymbols.push([reel, row]);
    }
  }
  ...
  const bonusWin = bonusSymbols.length >= 3 ? Math.floor(bet * 0.35 * (bonusSymbols.length - 2) * freeSpinMultiplier) : 0;
  const luckPay = Number(cfg.slotsV2.payouts.goodluck?.[Math.min(5, luckSymbols.length)]) || 0;
  const luckWin = luckSymbols.length >= 3 ? Math.floor(bet * luckPay * freeSpinMultiplier) : 0;
  return { lineWins, scatters, bonusSymbols, luckSymbols, scatterWin, bonusWin, luckWin, payout: lineTotal + scatterWin + bonusWin + luckWin };
}
