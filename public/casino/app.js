...
async function enter() {
  try {
    const data = await api("/session", "POST", { name: $("nameInput").value.trim() });
...
function renderBonus() {
  ...
  const now = Date.now();
  if (now >= readyAt) { btn.disabled = false; btn.textContent = `Claim Daily Bonus (+${fmt(config?.economy.dailyBonus ?? 500)})`; $("bonusTimer").textContent = "Available now!"; }
  else if (player.balance < (config?.economy.rescueThreshold ?? 50)) { btn.disabled = false; btn.textContent = `Claim Rescue Chips (+${fmt(config?.economy.rescueBonus ?? 250)})`; $("bonusTimer").textContent = "You're low on chips."; }
  else { btn.disabled = true; btn.textContent = "Daily Bonus"; const ms = readyAt - now; $("bonusTimer").textContent = `Next bonus in ${Math.floor(ms/3600000)}h ${Math.floor((ms%3600000)/60000)}m`; }
}
...
function affordable(bet) { if (bet > player.balance) { toast("Not enough chips", "lose"); return false; } return true; }
...
async function spinSlots() {
  ...
  try {
    ...
    await Promise.all(reels.map((r, i) => spinReel(r, o.reels[i], 1300 + i * 450)));
...
async function flipCoin() {
  ...
  try {
    ...
    inner.style.transition = "transform 1.1s cubic-bezier(.3,.1,.2,1)";
...
async function playCrash() {
  ...
  try {
    ...
    const dur = Math.min(5000, Math.max(700, 700 + 700 * Math.log(endM)));
    ...
    await wait(400); rocket.textContent = "🚀";
  } catch (e) { toast(e.message, "lose"); }
