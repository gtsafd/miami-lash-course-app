...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "POST" && req.url === "/api/redeem") return redeem(req, res);
    if (req.method === "GET" && req.url === "/api/course") return course(req, res);
    if (req.method === "POST" && req.url === "/api/progress") return progress(req, res);
    if (req.method === "POST" && req.url === "/api/admin/login") return adminLogin(req, res);
    if (req.method === "GET" && req.url === "/api/admin/state") return adminState(req, res);
    if (req.method === "POST" && req.url === "/api/admin/codes") return adminCreateCode(req, res);
    if (req.method === "PUT" && req.url === "/api/admin/course") return adminUpdateCourse(req, res);
    if (req.method === "DELETE" && req.url.startsWith("/api/admin/codes/")) return adminDeleteCode(req, res);
...
server.listen(port, () => {
  ...
  if (adminPassword === "change-this-admin-password") {
    console.log("Set ADMIN_PASSWORD before real deployment.");
  }
});
...
function adminState(req, res) {
  ...
  json(res, 200, publicAdminState(readStore()));
}
...
function createCode() {
  return `LASH-${crypto.randomBytes(2).toString("hex").toUpperCase()}-${crypto.randomBytes(2).toString("hex").toUpperCase()}`;
}
...
function lesson(id, poster, enTitle, ruTitle, enMeta, ruMeta, enNote, ruNote, enSteps, ruSteps) {
  return {
    ...
    poster,
    content: {
      en: { title: enTitle, meta: enMeta, note: enNote, steps: enSteps },
      ru: { title: ruTitle, meta: ruMeta, note: ruNote, steps: ruSteps }
    }
  };
}
...
function resource(id, enTitle, ruTitle, enText, ruText) {
  return {
    id,
    content: {
      en: { title: enTitle, text: enText },
      ru: { title: ruTitle, text: ruText }
    }
  };
}
