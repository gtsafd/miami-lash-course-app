...
async function publicConfig(req, res) {
  const store = await readStore();
  const cfg = mergeConfig(store.config);
  // Only expose what the client needs for UI (payouts, wheel segments, economy).
  json(res, 200, {
    config: {
      economy: cfg.economy,
...
async function adminState(req, res) {
  ...
  const store = await readStore();
  const cfg = mergeConfig(store.config);
  const totalChips = store.players.reduce((a, p) => a + (p.balance || 0), 0);
  const totalWagered = store.players.reduce((a, p) => a + (p.stats?.wagered || 0), 0);
  json(res, 200, {
    config: cfg,
    stats: { players: store.players.length, totalChips, totalWagered }
  });
}
...
function sanitizeConfig(input) {
  ...
  for (const s of SLOT_SYMBOLS) {
    merged.slots.weights[s] = clamp(merged.slots.weights[s], 0, 100000);
    merged.slots.triples[s] = clamp(merged.slots.triples[s], 0, 100000);
  }
  ...
  merged.wheel.segments = merged.wheel.segments.slice(0, 16).map((s) => ({
    ...
    multiplier: clamp(Number(s.multiplier), 0, 100000),
    weight: clamp(Number(s.weight), 0, 100000),
...
module.exports = {
  ...
  _internal: { handValue, freshDeck, rouletteWins, rouletteColor, normalizeRouletteBet, crashPointForEdge, weightedPick, mergeConfig, sanitizeConfig, defaultConfig }
};
